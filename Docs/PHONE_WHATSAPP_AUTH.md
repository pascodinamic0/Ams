...
| Step | Action |
|------|--------|
| **Apply for credits** | [Twilio for Startups](https://www.twilio.com/startups) � up to $5K if Digni Digital LLC qualifies (free to apply) |
| **Start WABA** | Meta Business Manager ? WhatsApp Business account (free to begin; verification takes 1�2 weeks) |
| **Supabase UI prep** | Dashboard ? Auth ? Providers ? Phone ? enable (Twilio fields can wait) |
| **Link test user phone** | `bun run seed:phone-auth-test` � sets `auth.users.phone` on hosted project |
| **Local end-to-end demo** | `supabase start` + test OTP below � no Twilio send |
...
2. Create a **Verify Service** (`VA�`) under Verify ? Services.
...
4. Create or reuse a **WhatsApp Sender** (WABA + phone number):
   ...
   - Complete Meta business verification (often 1�2 weeks)
   - Use a **dedicated** sender for OTPs (not the same number as fee reminders)
5. Create a **Messaging Service** (`MG�`) and attach the WhatsApp Sender.
...
- **Parents:** guardian record with `auth_user_id` + `phone` � synced automatically on guardian save.
- **Staff:** Settings ? **Phone for WhatsApp sign-in** � verify via OTP after linking.
...
| Error 63018 | WhatsApp messaging limit exceeded � check Meta limits |
| Error 63008 | Verify WhatsApp Messaging Service not configured |
| "No account" on login | Phone not linked on `auth.users` � use Settings or guardian sync |
