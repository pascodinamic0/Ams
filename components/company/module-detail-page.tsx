    ...
    <div className="min-h-screen bg-mkt-canvas pb-24 pt-32">
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <header className="mt-8 border-b border-mkt-ink/10 pb-10">
          <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-full border border-mkt-ink/15 text-amber-500">
            <Icon className="h-7 w-7" />
          </div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-amber-500">
            {module.tagline}
          </p>
          <h1 className="mt-3 font-display text-3xl tracking-tight text-mkt-ink md:text-5xl">
            {module.title}
          </h1>
        ...
        <section className="mt-10 space-y-6">
          ...
          <ul className="space-y-4">
            {module.highlights.map((item) => (
              <li
                key={item}
                className="flex items-start gap-3 text-base text-mkt-ink/60"
              >
        ...
        <section className="mt-12 space-y-4">
          ...
          <ul className="space-y-2">
            {module.whoItsFor.map((item) => (
              <li
                key={item}
                className="border border-mkt-ink/10 px-4 py-3 text-sm font-medium text-mkt-ink/65"
              >
                {item}
              </li>
            ))}
