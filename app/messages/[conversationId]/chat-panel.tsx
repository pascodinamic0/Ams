      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        {messages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            <p className="text-sm text-stone-400">{t("noMessagesSayHello")}</p>
          </div>
        ) : (
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"} mb-1`}>
                      <div className={`max-w-[70%] ${isMe ? "items-end" : "items-start"} flex flex-col`}>
                        {showName && (
                          <p className="mb-0.5 ml-1 text-xs font-medium text-stone-500">{displayName}</p>
                        )}
                        <div
                          className={`rounded-2xl px-4 py-2.5 text-sm shadow-sm ${
                            isMe
                              ? "rounded-br-sm bg-primary text-white"
                              : "rounded-bl-sm bg-stone-100 text-stone-900 dark:bg-stone-800 dark:text-white"
                          }`}
                        >
                          <p style={{ whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{msg.body}</p>
                        </div>
                        <p
                          className={`mt-0.5 text-xs text-stone-400 ${isMe ? "text-right" : "text-left"} px-1`}
                        >
      ...
      <div className="border-t border-stone-200 px-4 py-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] dark:border-stone-700">
        <div className="flex items-end gap-3">
          <textarea
            ...
            className="flex-1 resize-none rounded-xl border border-stone-300 bg-stone-50 px-4 py-2.5 text-sm text-stone-900 placeholder:text-stone-400 focus:border-primary-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-500/20 dark:border-stone-700 dark:bg-stone-800 dark:text-white dark:placeholder:text-stone-500 dark:focus:bg-slate-700"
          ...
          <button
            ...
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary text-white shadow-sm transition-all hover:bg-primary-hover disabled:opacity-40"
            aria-label={t("send")}
          >
        ...
        <p className="mt-1.5 text-xs text-stone-400">{t("typeMessageHint")}</p>
      </div>
    </div>
  );
}
