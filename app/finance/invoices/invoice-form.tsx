...
export function InvoiceForm({ students, feeStructures }: Props) {
  ...
  async function onSubmit(data: InvoiceFormData) {
    ...
    if (result.error) {
      toast.error(
        typeof result.error === "string" ? result.error : t("createInvoiceFailed")
      );
      return;
    }
...
function InvoiceFormFields({
  ...
  return (
    <>
      <div>
        <Label htmlFor="student_id" required>{t("colStudent")}</Label>
        <select
          id="student_id"
          {...register("student_id")}
      ...
      <div>
        <Label htmlFor="fee_structure_id">{t("colFeeType")}</Label>
        <select
          id="fee_structure_id"
          ...
          className="w-full rounded-lg border px-3 py-2 text-sm dark:border-stone-700 dark:bg-stone-900"
        >
          <option value="">{t("customAmount")}</option>
          {feeStructures.map((s) => (
      ...
      <div>
        <Label htmlFor="due_date" required>{t("colDueDate")}</Label>
      ...
      <div className="flex items-end sm:col-span-2 lg:col-span-1">
        <Button type="submit" className="w-full">{t("createInvoice")}</Button>
      </div>
    </>
  );
}
