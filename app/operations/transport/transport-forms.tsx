...
export function RouteForm({ branchId }: { branchId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    if (result.error) {
      toast.error(t("routeCreateFailed"));
      return;
    }
...
export function VehicleForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!routeId) {
      toast.error(t("createRouteFirst"));
      return;
    }
    setLoading(true);
    ...
    setLoading(false);
    if (result.error) {
      toast.error(t("vehicleAddFailed"));
      return;
    }
...
export function StudentMappingForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    if (result.error) {
      toast.error(t("assignStudentsFailed"));
      return;
    }
    toast.success(
      selectedIds.size === 1
  ...
  if (vehicles.length === 0 || students.length === 0) {
    return <p className="text-sm text-stone-500">{t("mappingPrerequisites")}</p>;
  }
      ...
      <Button type="submit" disabled={loading || selectedIds.size === 0}>
        {loading
          ...
          : selectedIds.size === 0
            ...
            : selectedIds.size === 1
              ? t("assignOneStudent")
              : t("assignNStudents", { count: selectedIds.size })}
      </Button>
    </form>
  );
}
...
export function DeleteRouteButton({ id }: { id: string }) {
  ...
  async function handleDelete() {
    ...
    setLoading(false);
    if (result.error) {
      toast.error(t("routeDeleteFailed"));
      return;
    }
...
export function DeleteVehicleButton({ id }: { id: string }) {
  ...
  async function handleDelete() {
    ...
    setLoading(false);
    if (result.error) {
      toast.error(t("vehicleDeleteFailed"));
      return;
    }
...
export function UnassignButton({ mappingId }: { mappingId: string }) {
  ...
  async function handleUnassign() {
    ...
    setLoading(false);
    if (result.error) {
      toast.error(t("unassignStudentFailed"));
      return;
    }
