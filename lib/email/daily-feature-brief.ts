/**
 * HTML email for daily feature factory — visual A→Z brief (founder-facing).
 * Style matches Docs/ShuleOS-Payment-Justification reports: navy kicker, numbered tour.
 */

import { escapeHtml, sendEmail, type SendEmailResult } from "@/lib/services/email";

export type DailyFeatureTourStep = {
  letter: string;
  caption: string;
  screenshotUrl?: string;
};

export type DailyFeatureBriefInput = {
  to: string;
  featureName: string;
  gapClosed: string;
  prUrl: string;
  previewUrl: string;
  videoUrl?: string;
  steps: DailyFeatureTourStep[];
  /** Idempotency segment (e.g. pr-42-run-123) */
  runDate: string;
  prNumber: number;
};

type BriefPayload = Pick<
  DailyFeatureBriefInput,
  "featureName" | "gapClosed" | "prUrl" | "previewUrl" | "videoUrl" | "steps"
>;
...
export function buildDailyFeatureBriefHtml(input: BriefPayload): string {
...
export function buildDailyFeatureBriefText(input: BriefPayload): string {
  const lines = [
...
export async function sendDailyFeatureBrief(
  input: DailyFeatureBriefInput
  ...
  return sendEmail({
    ...
    html: buildDailyFeatureBriefHtml(payload),
    text: buildDailyFeatureBriefText(payload),
    idempotencyKey: `daily-feature-brief/${input.runDate}/pr-${input.prNumber}`,
  });
}
